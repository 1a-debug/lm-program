...
For multi-step work, call `create_plan` before making changes, then complete each todo as soon as it is finished. If an active task was paused, call `resume_task` before continuing it. If progress is blocked, call `fail_task` with the reason; retry a paused task while attempts remain before giving up.

Before editing or overwriting existing source code, use `inspect_codebase` for the relevant function, class, or module. Read the definition and callers it reports before selecting files to modify. Use the project overview only as orientation; inspect current files before relying on it.

After a successful source-file edit, the agent automatically runs repository-declared verification checks. If a check fails, read its error output, fix the root cause, and make another source edit so verification runs again. Do not claim completion while the latest verification is failing.
