        ...
        for turn_num in range(max_turns):
            ...
            if self.session.context_manager.needs_compression():
                yield AgentEvent.compaction_start(
                    self.session.context_manager.message_count
                )
                ...
                if summary:
                    ...
                    yield AgentEvent.compaction_complete(
                        self.session.context_manager.message_count,
                        usage,
                    )
                else:
                    yield AgentEvent.compaction_error(
                        self.session.chat_compactor.last_error
                        or "Compaction failed."
                    )
            ...
            for tool_call in tool_calls:
                ...
                if (
                    ...
                    for check in discover_verification_checks(self.config.cwd):
                        ...
                        verification = await self.session.tool_registry.invoke(
                            "shell",
                            {"command": check.command, "timeout": 120},
                            self.config.cwd,
                            self.session.hook_system,
                        ...
                        details = verification.output or verification.error or "No output"
                        verification_output.append(
                            f"{check.name} ({check.command}): {status}\n{details}"
                        )
                    if verification_output:
                        result.output += "\n\nAutomatic verification:\n" + "\n\n".join(verification_output)
                        result.metadata["verification_ran"] = True
